...
const CharacterCreationControlDisplay = () => {
  const { sendButtonInput: sendButtonInput, choiceContext } =
  ...
  useEffect(() => {
    if (choiceContext) {
      ...
      const _origins = choiceContext.playerChoices.filter(
        (x: any) => x.choiceContext === "origin"
      );
      setOrigins(_origins);
    }
  }, [choiceContext]);
      ...
      return (
        <GridItem>
          <OriginSelectionButton
            selected={
              selectedOrigin ? selectedOrigin.name === origin.name : false
            }
            onSelect={() => {
            ...
            confirm={() => {
              sendButtonInput("origin", origin.name);
            }}
            origin={origin}
          />
        </GridItem>
    ...
    <>
      ...
      <Accordion allowToggle p={4} bg={"blackAlpha.200"}>
        <AccordionItem>
          ...
          <AccordionPanel pb={4}>{pronounSelection()}</AccordionPanel>
        </AccordionItem>
        <AccordionItem>
          <h2>
            <AccordionButton _expanded={_expanded}>
          ...
          <AccordionPanel pb={4}>{originSelection(pronoun)}</AccordionPanel>
        </AccordionItem>
        ...
        <AccordionItem>
          ...
          <AccordionPanel pb={4} pl={1}>
            <SimpleGrid columns={isMobile ? 1 : 4}>
              <GridItem colSpan={3}>{colorSelection()}</GridItem>
              <GridItem>
                <Center>
